...
def _parse_rows_from_sheet(ws, sheet_type: str) -> list:
    """워크시트 → rows 리스트 변환. sheet_type: 'ojc'(구매조회) or 'purchase'(구매현황)"""
    rows = []
    for r in range(3, ws.max_row + 1):
        if sheet_type == 'ojc':
            # 구매조회: col1=구매번호(날짜), col5=품목명[규격명], col6=수량합계
            buy_no = ws.cell(r, 1).value
            pr     = ws.cell(r, 5).value
            qty    = ws.cell(r, 6).value
            pc     = ''
        else:
            # 구매현황: col1=날짜, col5=품목코드, col6=품목명[규격명], col7=수량
            buy_no = ws.cell(r, 1).value
            pc     = ws.cell(r, 5).value
            pr     = ws.cell(r, 6).value
...
def preprocess(path: str, logs: list = None) -> Workbook:
    ...
    if '구매조회' in sheets:
        ...
        rows = _parse_rows_from_sheet(wb_in['구매조회'], 'ojc')
    elif '구매현황' in sheets:
        logs.append("구매현황 형식 감지 → 자동 변환 시작")
        rows = _parse_rows_from_sheet(wb_in['구매현황'], 'purchase')
    else:
        wb_in.close()
    ...
    if not rows:
        raise ValueError("파싱된 데이터가 없습니다. 파일 형식을 확인해주세요.")
