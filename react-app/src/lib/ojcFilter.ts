...
// KT-SP/DP/다심, LG-S/D/M, 피그테일-성단용 세분화
export function classifyOjcDetailed(name: string): string | null {
  if (name.startsWith('OJC-A1-') || name.startsWith('OJC-C2-')) {
    ...
    if (name.includes('다심')) return 'KT OJC-다심'
    return 'KT OJC'
  }
  if (name.startsWith('SOJC-')) return 'LG OJC-S'
  if (name.startsWith('DOJC-')) return 'LG OJC-D'
  if (name.startsWith('MOJC-')) return 'LG OJC-M'
  if (name.startsWith('DROP-CABLE')) return 'DROP'
  if (name.startsWith('PIGTAIL-')) return name.includes('성단') ? '피그테일-성단용' : '피그테일'
