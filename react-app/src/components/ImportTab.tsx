  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#f0f4fa] border-l-4 border-[#2E75B6] px-5 py-4 rounded-md text-sm">
        <b>수입 관리</b> — EMP 코드 생성기 · 업체코드 관리
        <span className="text-gray-500 ml-2">
          EMP 로트 코드 형식: <code className="bg-white px-1 rounded border text-xs">기본코드 (업체-YYMM-발주차수_입고회차)</code>
        </span>
      </div>
      ...
      <div className="flex items-center gap-4 bg-white border border-gray-200 rounded-lg px-5 py-3">
        <div className="flex-1">
          ...
          <div className="text-xs text-gray-400 mt-0.5">
            {itemCodes.length > 0
              ? `${itemCodes.length}개 품목 등록됨`
              : '이카운트에서 품목 전체 내보내기(.xlsx)를 업로드하면 기본코드 드롭다운이 채워집니다'}
          </div>
        </div>
        <input
          ref={fileInputRef}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="border border-gray-200 rounded-lg p-5 space-y-4">
          <h2 className="text-sm font-semibold text-[#1F3864]">🏷 EMP 코드 생성기</h2>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-[#2E75B6]"
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map(c => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-1.5 text-xs hover:bg-[#f0f4fa] ${itemCode === c.code ? 'bg-[#e8f4fd] font-semibold text-[#2E75B6]' : ''}`}
                    >
            ...
            {itemCode && (
              <div className="mt-1 text-xs text-[#2E75B6] font-semibold">선택됨: {itemCode}</div>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded px-3 py-1.5 text-sm text-center font-mono focus:outline-none focus:border-[#2E75B6]"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 rounded px-2 py-1.5 text-sm text-center focus:outline-none focus:border-[#2E75B6]"
              >
          ...
          {/* 결과 미리보기 */}
          <div className={`rounded-lg border-2 p-4 transition-colors ${result ? 'border-[#2E75B6] bg-[#e8f4fd]' : 'border-dashed border-gray-300 bg-gray-50'}`}>
            <div className="text-xs text-gray-500 mb-1">생성 결과</div>
            <div className={`font-mono text-base font-bold ${result ? 'text-[#1F3864]' : 'text-gray-400'}`}>
          ...
          <button
            onClick={handleCopy}
            disabled={!result}
            className="w-full py-2 rounded bg-[#2E75B6] hover:bg-[#1a5a9e] disabled:bg-gray-300 text-white text-sm font-semibold transition"
          >
            {copied ? '✅ 복사됨!' : '📋 복사'}
          </button>
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {history.map((h, i) => (
                  <button
                    ...
                    className="w-full text-left px-2 py-1 text-xs font-mono bg-gray-50 hover:bg-[#f0f4fa] border border-gray-200 rounded"
                  >
                    {h}
                  </button>
                ))}
        ...
        <div className="border border-gray-200 rounded-lg p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {vendorMsg && <span className="text-xs text-[#375623]">{vendorMsg}</span>}
          </div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {vendors.map((v, i) => (
                editingCode === v.code ? (
                  <tr key={v.code} className="bg-[#ffffc0]">
                    ...
                    <td className="px-2 py-1">
                      <div className="flex gap-1">
                        <button onClick={() => handleVendorSave(newCode, newName)}
                          className="px-2 py-1 text-xs bg-[#375623] text-white rounded hover:bg-[#2a3f1a]">저장</button>
                  ...
                  <tr key={v.code} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-3 py-2 font-mono font-bold text-[#2E75B6] text-xs">{v.code}</td>
                    <td className="px-3 py-2 text-gray-700 text-xs">{v.name}</td>
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        <button
                          onClick={() => { setEditingCode(v.code); setNewCode(v.code); setNewName(v.name) }}
                          className="px-2 py-1 text-xs border border-gray-300 rounded hover:bg-gray-100"
                        >수정</button>
                        <button
                          onClick={() => handleVendorDelete(v.code)}
                  ...
                  <td className="px-2 py-1">
                    <div className="flex gap-1">
                      <button onClick={() => handleVendorSave(addCode, addName)}
                        className="px-2 py-1 text-xs bg-[#2E75B6] text-white rounded hover:bg-[#1a5a9e]">추가</button>
          ...
          {!addMode && (
            <button
              onClick={() => { setAddMode(true); setEditingCode(null) }}
          ...
          <div className="bg-[#f0f4fa] rounded p-3 text-xs text-gray-500 space-y-0.5">
            <div className="font-semibold text-gray-600 mb-1">EMP 로트 코드 형식</div>
            <div><code className="bg-white px-1 rounded border">기본코드 (업체코드-YYMM-발주차수_입고회차)</code></div>
            <div className="mt-1">예시: <code className="bg-white px-1 rounded border">14-K-107 (FLC-2601-01_02)</code></div>
