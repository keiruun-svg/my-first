    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab('cable')}
          className={`px-4 py-2 rounded font-semibold text-sm transition ${tab==='cable' ? 'bg-[#2E75B6] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
          케이블 ({cableKeys.length})
        </button>
        <button onClick={() => setTab('housing')}
          className={`px-4 py-2 rounded font-semibold text-sm transition ${tab==='housing' ? 'bg-[#2E75B6] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
